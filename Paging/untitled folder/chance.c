...
void secondChance()
{
	int victimSelected=0; // victimSelected : Flag
	...
	while(!victimSelected)
	{
		...
		//CHANCE_GIVEN means one chance to that page is given already
		if(mem[victim][CHANCE_GIVEN] == 1)
			{
				victimSelected = 1;
				mem[victim][DATA] = -1; // victim page swapped out
			}	
	
		else if(mem[victim][CHANCE_GIVEN] == 0) 
			{
				mem[victim][CHANCE_GIVEN] =1; // chance given
				victimSelected = 0;
			}	
	}//while(victime in not selected)
		
	printf("\nSecond Chance: Victim Frame Evacuated: ");
	print_memory();

}//second chance fun ends here
