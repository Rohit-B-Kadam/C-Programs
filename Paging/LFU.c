//This Algorithm is Out of Practical Syllabus
#include<stdio.h>
...
void LFU()
{
	int i, minimum=mem[0][REF_COUNT], minCountIndex=0;
	for(i=0; i<no_of_frames; i++)
	{
		if(mem[i][REF_COUNT] < minimum)
		{
			minimum = mem[i][REF_COUNT];
			minCountIndex = i;
		}
	}	
	
	victim = minCountIndex;
	
	mem[victim][DATA] = -1; // swap out victim page
	printf("\nLFU: Victim Frame Evacuated: ");
	print_memory();
}
